...
import tfd.system.unit.userinfo.logic.UserInfoImportLogic;
...
public class UserInfoImportAct {
	private UserInfoImportLogic logic=new UserInfoImportLogic();
	/**
	 * 数据导入 模板下载
	 * @param request
	 * @param response
	...
	public String templateDownload(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		try {
			...
			for(String column:template.keySet()){
				if (i==0) {
					record.addField(column,"初始密码为："+new AccountLogic().getImportInitPasswordClear());
				}else{
			...
			String exportFileName="用户信息导入模板.xls";
	...
	public String dataImport(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		try {
			...
			logic.account=(Account) request.getSession().getAttribute("ACCOUNT_ID");
		...
		return "/system/unit/userinfo/import.jsp";
	}
	/**
	 * 数据导入要求
	 * @param request
