...
public class ServletTool {
	...
	private static List<String> gbkCodeUriList = new ArrayList<String>();
	  ...
	  public static void setNeedNoCheckUriList(String uriStr) {
	    if (SysTool.isNullorEmpty(uriStr)) {
	      return;
	    }
	   ...
	   * @throws IOException
	   */
	  public static void forward(String toUrl, HttpServletRequest request,HttpServletResponse response) throws ServletException,IOException {
	  ...
	  public static boolean isLoginAction(HttpServletRequest request) {
	    ...
	    if (qUri.equals(contextPath + "/login.jsp")
	        ...
	        || qUri.startsWith(contextPath + "/com/system/login/act/DologinAct/LoginInTo.act")
	        || qUri.startsWith(contextPath + "/com/system/login/act/DologinAct/doLogout.act")
	        || qUri.startsWith(contextPath + "/com/system/login/act/DologinAct/ImLoginInTo.act")
	        || qUri.startsWith(contextPath + "/mobile/act/AndroidAct/LoginIntoAct.act")
	        || qUri.startsWith(contextPath + "/mobile/act/AndroidAct/setLogin.act")
	        || qUri.startsWith(contextPath + "/tfd/system/sysinfo/")) 
	    {
	    	
	    	return true;
	    }
	    if (!SysTool.isNullorEmpty(SysProps.getString("NEED_NOT_CHECKED_URI"))) {
	  ...
	  public static boolean isGbkCode(HttpServletRequest request) {
	    ...
	    if (gbkCodeUriList.size() > 0 || !SysTool.isNullorEmpty(SysProps.getString("NEED_GBKCODE_URI"))) {
	  ...
	  public static String getUrlParams(HttpServletRequest request,
	    ...
	    while (eParamName.hasMoreElements()) {
	      ...
	      String paramValue = SysTool.iso88591ToGbk(
	  ...
	  public static Class applicationClass(String className) throws ClassNotFoundException {
	      ...
	      if (classLoader == null) {
	        classLoader = ServletTool.class.getClassLoader();
	      }
	  ...
	  public static String getParam(HttpServletRequest request, String paramName, String defalutValue) {
	    ...
	    Map paramMapInner = (Map)request.getAttribute(SysConst.PARAM_MAP_INNER);
	    if (paramMapInner != null) {
	      paramValue = (String)paramMapInner.get(paramName);
	      if (!SysTool.isNullorEmpty(paramValue)) {
	        return paramValue;
	      }
	    }
