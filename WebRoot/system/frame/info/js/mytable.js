	...
	for(var i=0;i<colArray.length;i++)
	{
		for(var j=0;j<colArray[i].childNodes.length-1;j++)
		{
			...
			if(body_i)
			{
			   ...
			   body_i.onmouseover = function() {if($(this.module.id+"_type_op") && $(this.module.id+"_type_link") && $(this.module.id+"_type_link").scrollHeight > $(this.module.id+"_type_link").clientHeight) $(this.module.id+"_type_op").style.display="inline";};
			   body_i.onmouseout  = function() {if($(this.module.id+"_type_op")) $(this.module.id+"_type_op").style.display="none";};
		   }
		   
		   var type_link_i = $(module_i.id+"_type_link");
			if(type_link_i)
			{
			   for(var k=0; k< type_link_i.childNodes.length; k++)
			   {
			      if(!type_link_i.childNodes[k].tagName || type_link_i.childNodes[k].tagName.toUpperCase() != "A")
			         continue;
			      
			      if(k == 0)
			         type_link_i.childNodes[k].className="active";
			      
			      type_link_i.childNodes[k].onclick = function()
			      {
			         var links = this.parentNode.childNodes;
			         for(var i=0; i< links.length; i++)
			         {
			            if(links[i].tagName && links[i].tagName.toUpperCase() == "A")
			               links[i].className="";
			         }
		   ...
		   fixTitleWidth(module_i.id)
			...
			title_i.onDragStart=function(af,ag) {
					...
					module_i_copy.style.left=getOffset(module_i,true) + 'px';
					module_i_copy.style.top=getOffset(module_i,false) + 'px';
...
{
	if(!$(module_id)) return;
	if($(module_id+"_head").clientWidth - $(module_id+"_text").scrollWidth - 120 - 40 > 0)
	   $(module_id+"_title").style.width=$(module_id+"_head").clientWidth - $(module_id+"_text").scrollWidth - 120 - 40 + "px";
	if($(module_id+"_head").clientWidth - $(module_id+"_text").scrollWidth - 120 - 40 > 0)
	   $(module_id+"_title").style.width=$(module_id+"_head").clientWidth - $(module_id+"_text").scrollWidth - 120 - 40 + "px";
	
	if(!$(module_id+"_type_link") || !$(module_id+"_head")) return;
	$(module_id+"_type_link").style.width = Math.max(0, $(module_id+"_head").clientWidth - 60) + "px";
}

function ScrollType(id)
{
   var obj = $("module_"+id+"_type_link");
   if(!obj) return;
   var height = obj.clientHeight;
   if(obj.scrollTop + height >= obj.scrollHeight || obj.scrollTop + height*2 > obj.scrollHeight)
      obj.scrollTop = 0;
   else
...
function aD(a,aa,ab)
{
	...
	return setInterval(function(){if(ab<1){clearInterval(ap);ax();fixTitleWidth(a.id);return}ab--;ac-=ae;ad-=af;createDiv().style.left=parseInt(ac)+"px";createDiv().style.top=parseInt(ad)+"px"},aa/ab)
}
...
function get_module_content(module_id, url)
{
   $("module_"+module_id+"_ul").innerHTML="<div style='text-align:center;'><img src='/static/images/loading.gif' align='absMiddle'> �����У����Ժ򡭡�</div>";
   var xmlHttpObj=getXMLHttpObj();
   ...
   xmlHttpObj.onreadystatechange=function()
	{
	 	if(xmlHttpObj.readyState==4)
	 	{
	 		if(xmlHttpObj.status==200)
	 		   $("module_"+module_id+"_ul").innerHTML=xmlHttpObj.responseText;
	 		else
	 		   $("module_"+module_id+"_ul").innerHTML=td_lang.inc.msg_103+xmlHttpObj.status;//"�������ݴ��󣬴��룺"
	 	}
	}
   xmlHttpObj.send(null)
}
...
{
   var o=$('module_'+module_id+'_ul');
   if(!o)   return;
   o.scrollTop+=page*lines_per_page(module_id)*20;
   if(o.scrollTop > 0)
      $('module_'+module_id+'_link_pre').className="page-link-prev";
   else
      $('module_'+module_id+'_link_pre').className="page-link-prev PageLinkDisable";
   if(o.scrollHeight-o.scrollTop>lines_per_page(module_id)*20)
      $('module_'+module_id+'_link_next').className="page-link-next";
   else
      $('module_'+module_id+'_link_next').className="page-link-next PageLinkDisable";
}
...
function CorrectImg()
{
   ...
   for(var i=0; i<imgs.length; i++)
   {
      ...
      if(tagName!="li" && tagName!="a")
         continue;
      if(el.parentElement.parentElement && (el.parentElement.parentElement.className=="close" || el.parentElement.className=="expand"))
         continue;
      
      var padding_top=Math.floor((20-el.height)/2);
      var padding_bottom=20-el.height-padding_top;
      padding_top=padding_top<0 ? 0:padding_top;
      padding_bottom=padding_bottom<0 ? 0:padding_bottom;
      el.style.padding=padding_top+"px 3px "+padding_bottom+"px 3px";
   }
   getinfo();
}
function doinit(){
...
function updateAddress(){
	var col_l = jQuery("#col_l .module_div");
	for(var i = 1 ; i < col_l.size() ; i++){
		var id = col_l.eq(i-1).find(".homePageId").val();
		doUpdate(id,1,i);
	}
	var col_r = jQuery("#col_r .module_div");
	for(var i = 1 ; i < col_r.size() ; i++){
		var id = col_r.eq(i-1).find(".homePageId").val();
		doUpdate(id,2,i);
	}
}
function doUpdate(id,addressTd,addressTr){
	var requestUrl=contextPath+'/tfd/system/homepage/act/HomePageAct/updateHomePage.act';
	jQuery.ajax({
		url:requestUrl,
		data:{id:id,addressTd:addressTd,addressTr:addressTr},
		dataType:"text",
		async:false,
		type:"POST",
		error:function(e){
