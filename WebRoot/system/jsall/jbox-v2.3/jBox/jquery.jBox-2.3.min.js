	...
	a.jBox = function(b, c) {
		...
		if (c.showFade) {
			if ((d && a('iframe').length > 0x0) || a('object, applet').length > 0x0) {
				j.push('<iframe id="jbox-fade" class="jbox-fade" src="about:blank" style="display:block;position:absolute;z-index:99"></iframe>');
			} else {
				if (d) {
					a('select').css('visibility', 'hidden');
				};
				j.push('<div id="jbox-fade" class="jbox-fade" style="position:absolute;z-index:99"></div>');
			}
		};
		j.push('<div id="jbox-temp" class="jbox-temp" style="z-index:99;width:0px;height:0px;background-color:#ff3300;position:absolute;display:none;"></div>');
		if (c.draggable) {
			j.push('<div id="jbox-drag" class="jbox-drag" style="position:absolute;display:none;"></div>');
		};
		...
		a.each(b, function(N, O) {
			...
			r.push('<div class="jbox-button-panel" style="height:25px;padding:0px 0 5px 0;text-align: right;' + (P ? '' : 'display:none;') + '">');
			if (!c.isTip) {
