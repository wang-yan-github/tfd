function Designer(container) {
	...
	this.drawStart = function(x, y, info) {
		var node = $('<div node start noWrap class="node start_node"><img src="'
				+ contextPath
				+ '/system/styles/images/workflow/start_node.png" /><br/><b>'
				+ info.prcsName + "</b></div>");
	...
	this.drawEnd = function(x, y, info) {
		var node = $('<div node end noWrap class="node end_node"><img src="'
				+ contextPath + '/system/styles/images/workflow/end_node.png" /><br/>'
				+ info.prcsName + "</div>");
		node.attr("id", info.sid);
		node.data("data", info);
		node.css({
	...
	this.drawSimpleNode = function(x, y, info) {
	    var prcsName="";
	    if(info.prcsName.length>5)
	    {
	        prcsName=info.prcsName.substring(0,5); 
	    }else
	    {
	        prcsName=info.prcsName;
	    }
		var node = $('<div node simple noWrap class="node simple_node"><img src="'
				+ contextPath
				+ '/system/styles/images/workflow/simple_node.png" /><br/>'
				+ prcsName+ "</div>");
		node.attr("id", info.sid);
		node.data("data", info);
		node.css({
	...
	this.drawAggregationNode = function(x, y, info) {
		var node = $('<div node aggregation noWrap class="node aggregation_node"><img src="'
				+ contextPath
				+ '/system/styles/images/workflow/aggregation_node.png" /><br/>'
				+ info.prcsName + "</div>");
		node.attr("id", info.sid);
		node.data("data", info);
		node.css({
	...
	this.drawParallelNode = function(x, y, info) {
		var node = $('<div  node parallel noWrap class="node parallel_node"><img src="'
				+ contextPath
				+ '/system/styles/images/workflow/parallel_node.png" /><br/>'
				+ info.prcsName + "</div>");
		node.attr("id", info.sid);
		node.data("data", info);
		node.css({
	...
	this.drawChildNode = function(x, y, info) {
		var node = $('<div  node child noWrap class="node child_node"><img src="'
				+ contextPath
				+ '/system/styles/images/workflow/child_node.png" /><br/>'
				+ info.prcsName + "</div>");
		node.attr("id", info.sid);
		node.data("data", info);
		node.css({
