	...
	layer.prompt = function(a, c) {
		...
		return layer.open($.extend({
			...
			title : function() {
				...
				if (a > 0)
					for ( d = '<span class="layui-layer-tabnow">' + c[0].title + "</span>"; a > b; b++)
						d += "<span>" + c[b].title + "</span>";
				return d;
			}(),
			content : '<ul class="layui-layer-tabmain">' + function() {
				var a = c.length, b = 1, d = "";
				if (a > 0)
					for ( d = '<li class="layui-layer-tabli xubox_tab_layer">' + (c[0].content || "no content") + "</li>"; a > b; b++)
						d += '<li class="layui-layer-tabli">' + (c[b].content || "no  content") + "</li>";
				return d;
			}() + "</ul>",
			success : function(a) {
		...
		if ( a = a || {}, a.photos) {
			...
			f.imgprev = function(a) {
				...
				layer.close(f.loadi), f.index = layer.open($.extend({
					...
					content : '<div style=\"height:'+h+'px;line-height:'+h+'px;\" class="layui-layer-phimg"><img style=\"height:'+img_h+'px\"  src="' + i[j].src + '" alt="' + (i[j].alt || "") + '" layer-pid="' + i[j].pid + '"><div class="layui-layer-imgsee"><span class="layui-layer-imguide"><a href="javascript:;" class="layui-layer-iconext layui-layer-imgprev"></a><a href="javascript:;" class="layui-layer-iconext layui-layer-imgnext"></a></span><div class="layui-layer-imgbar" style="display:' + ( d ? "block" : "") + '"><span class="layui-layer-imgtit"><a href="javascript:;">' + (i[j].alt || "") + "</a><em>" + f.imgIndex + "/" + i.length + "</em></span></div></div></div>",
				...
				if(c.height>400){
					$(".layui-layer-photos").css("maxHeight",$(window).height()-100);
					$(".layui-layer-content").css("maxHeight",$(window).height()-100);
					$(".layui-layer-photos").css("top","46px");
				}
