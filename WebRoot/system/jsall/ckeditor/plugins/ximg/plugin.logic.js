var flowform_plugin_ximg=new Object();
$(function() {
	...
	//校验，required
	flowform_plugin_ximg.validate=function() {
	...
	// 转换dom节点，required
	flowform_plugin_ximg.toDomStr=function() {
		var title = $("#title").val();
		var fieldName = $("#fieldName").val();
		var width =  $("#width").val();
		var height=$("#height").val();
		var html = "<img " +
				   "	src='"+contextPath+"/system/styles/images/workflow/imgup.png'"+
				   "	xtype='ximg' "+
				   "	datatype='text' " +
				   "	name='" + fieldName + "' " +
				   "	id='" + fieldName + "'"+
	...
	if (element != null && element.getAttribute('xtype') == 'ximg') {
		...
		if (element.getAttribute('width')) {
			$("#width").val(element.getAttribute('width'));
		}
		if (element.getAttribute('height')) {
			$("#height").val(element.getAttribute('height'));
		}
	}
