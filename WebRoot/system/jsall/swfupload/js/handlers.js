...
function queueComplete(numFilesUploaded) {
	...
	//status.html("<div>"+numFilesUploaded + " 个文件" + (numFilesUploaded === 1 ? "" : "s") + "已上传.</div>"); 
}
...
function uploadSuccess(file, serverData) {
    try {
        ...
        progress.toggleCancel(false);
        var value = eval("(" + serverData + ")");
        var Id = value[0].attachmentId + ",";
        var Name = value[0].attachmentName + ",";
        var divId = value[0].divId;
        var module = value[0].module;
        if(divId!=="")
        {
            if(attachTmp[divId]!=undefined&&attachTmp[divId]!="undefined")
            {
                attachTmp[divId]+=value[0].attachmentId+",";
            }else
            {
                attachTmp[divId]=value[0].attachmentId+",";
            }
        }
        var extName=value[0].attachmentName.substring(value[0].attachmentName.lastIndexOf(".")+1);
		var name = value[0].attachmentName.substring(0,value[0].attachmentName.lastIndexOf("."));
		if(name.length > 14){
        ...
        var html="";
        html+="<div id=\""+value[0].attachmentId+"\" style='width:100%;height:25px;line-height:25px;'>\n"+
                    "<img style=\"float:left;width:20px;height:20px;\" src=\""+imgPath+"/filetype/file_extension_"+extName+".png\" />\n";
        html+="<span style=\"float:left;\" title=\""+value[0].attachmentName+"\" onmouseout=\"javascript:hideMenus()\" onmouseover=\"showMenu('"+value[0].attachmentId+"','"+extName+"','2','"+inputId+"')\" >"+name+"</span>\n";
        html+="</div>";
        if(divId==""||divId=="null")
        {
            var value=$("#attachId").val()+Id;
            var name=$("#attachName").val()+Name;
            $("#attachId").val(value);
            $("#attachName").val(name);
...
{
    var attachIds=$("#publicFileId").val();
    var url=contextPath+"/tfd/system/workflow/flowrun/act/FlowRunAct/updataAttachIdAct.act";
    $.ajax({
        url:url,
        type:"POST",
        dataType:"json",
        data:{runId:runId,attachIds:attachIds},
        async:false,
        error:function(e){
        },
