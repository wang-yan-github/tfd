(function(){
	...
	var getScript = function(options){
		...
		options = options || {};
		Zeus[callback] = options.success || function(){};
		...
		script.src = options.url + ((/\?/).test(options.url) ? '&' : '?') + 'callback=Zeus.' + callback;
	...
	Zeus.prototype.initWebSocket = function(node){
		var that = this;
	...
	Zeus.prototype.runHeartbeatTask = function(){
		var that = this;
	...
	Zeus.prototype.send = function(data){
		if(this.proto == 1){
			this.ws.send(data);
		}else{
			// Comet TODO
	...
	Zeus.prototype.getOfflineMessage = function(){
		var that = this;
		getScript({
	...
	Zeus.prototype.destory = function(){
		this.ws.close();
	};
