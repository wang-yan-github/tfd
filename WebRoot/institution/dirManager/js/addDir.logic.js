...
function add(){
	var requestUrl=contextPath+'/institution/act/DirectoryAct/addDirectory.act';
	var name = $('#directory').val();
	var topId = $('#directory_id').val();
	if(confirm("确认添加吗？")){
		$.ajax({
			url:requestUrl,
			data:{name:name,topId:topId},
			dataType:"json",
			async:false,
			error:function(e){
				alert(e.message);
			...
			success:function(data){
				if(data == "1"){
					top.layer.msg("添加成功");
					parent.document.getElementById("left").src = "/tfd/institution/dirManager/left.jsp";
...
function back(){
	window.location.href = "/tfd/institution/dirManager/right.jsp";
}
