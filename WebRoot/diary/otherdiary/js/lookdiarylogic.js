							...
							if((Date.parse(startTime)<=Date.parse(title))&&(Date.parse(endTime)>=Date.parse(title)||endTime=="")){
								...
								if(Date.parse(title)>=(Date.parse(mydate)-lockDay*24*60*60*1000)||lockDay==0){
									...
									if(beforestaff==parm[i].ACCOUNT_ID){
						 			divcon+="<a href=\"javascript:void(0);\" onclick=\"selectdiaryId('"+parm[i].DIARY_ID+"');\">编辑</a>&nbsp;&nbsp;&nbsp;<a href=\"javascript:void(0);\" onclick=\"deleteDiary('"+parm[i].DIARY_ID+"');\">删除</a>&nbsp;&nbsp;&nbsp;";
						}
								}else{
									if(commStatus==1){
									divcon+="<a href=\"javascript:void(0);\" onclick=\"gaindiaryComm('"+parm[i].DIARY_ID+"');\">评论<span id=\"Comment"+parm[i].DIARY_ID+"\">("+parm[i].COMM_COUNT+")</span></a>&nbsp;&nbsp;&nbsp;";
				...
				// divcon+="<div class=\"list-group-item\" id=\"div"+parm[i].DIARY_ID+"\" style=\"padding: 0px;cursor: auto;\">"+
				     ...
				     // "</div>"+
			          ...
			          // "<tr><td><div id='diarycontent"+parm[i].DIARY_ID+"' class=\"diarycontent\">"+parm[i].DIARY_CONTENT+"</div></td></tr>"
						...
						// "<span id=\"staff"+parm[i].DIARY_ID+"\"></span>"+
						// "&nbsp;&nbsp;&nbsp;<a href=\"javascript:void(0);\"  onclick=\"clearstaff('"+parm[i].DIARY_ID+"');\">X</a></div></div>"+
						// "<textarea rows=\"4\" id=\"commContect"+parm[i].DIARY_ID+"\" name=\"commContect\" class=\"form-control\"></textarea></form>"+
						// "<div class=\"remind\" id='remind"+parm[i].DIARY_ID+"'></div><div id=\"btnclick\">"+
						// "<button type=\"button\" class=\"btn btn-primary\" onclick=\"commentsubmit('"+parm[i].DIARY_ID+"','"+parm[i].ACCOUNT_ID+"');\">"+
						// "评论</button></div><br><br><div id=\"commtable"+parm[i].DIARY_ID+"\" class=\"diarycomments\"></div></div>"+
						// "</div></br>";
			}
...
function init(){
	var url=contextPath + "/diary/act/DiaryAct/selectotherDiaryAct.act";
	 ...
	 $(window).scroll(function (){
	        ...
	        if (scrollTop + windowHeight == scrollHeight) {
	        	...
	        	if(i==0){
	        		...
	        		url=contextPath+"/diary/act/DiaryAct/othertitletimeQueryDiaryAct.act";
