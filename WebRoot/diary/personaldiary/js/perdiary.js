...
//						}
//						divcon+="<div class=\"lookstaff\" id=\"lookstaff"+parm[i].DIARY_ID+"\">"+lookstaffName+"</div>"+
//						"<form id=\"comments"+parm[i].DIARY_ID+"\"> <div id=\"comm"+parm[i].DIARY_ID+"\" class=\"commtable\">"+
//						"<input id=\"commPid"+parm[i].DIARY_ID+"\" name=\"commPid\" value=\"0\" type=\"hidden\"></input>"+
//						"<input name=\"diaryId\" value='"+parm[i].DIARY_ID+"' type=\"hidden\"><div id=\"staffdiv"+parm[i].DIARY_ID+"\" style=\"display: none;\"><div class=\"presentstaff form-control\">回复："+
//						"<span id=\"staff"+parm[i].DIARY_ID+"\"></span>&nbsp;&nbsp;&nbsp;<a href=\"javascript:void(0);\"  onclick=\"clearstaff('"+parm[i].DIARY_ID+"');\">X</a></div></div>"+
//						"<textarea rows=\"4\" id=\"commContect"+parm[i].DIARY_ID+"\" name=\"commContect\" class=\"form-control\"></textarea></form>"+
//						"<div class=\"remind\" id='remind"+parm[i].DIARY_ID+"'></div><div id=\"btnclick\"><button type=\"button\" class=\"btn btn-primary\" "+
//						"onclick=\"commentsubmit('"+parm[i].DIARY_ID+"','"+parm[i].ACCOUNT_ID+"');\">评论"+
//						"</button></div><br><br><div id=\"commtable"+parm[i].DIARY_ID+"\" class=\"diarycomments\"></div></div>"+
//						"</div></br>";
			}
		...
		success : function(data) {
			...
			for(var i=0;i<data.length;i++){
				if(data[i].commPid=="0"){
					...
					"</a></div><div style=\"float:right\"><sapn style=\"color:#B4B4B4\">"+data[i].commTime+"</sapn>&nbsp;&nbsp;"+
					"<a href=\"javascript:void(0);\" onclick='choicestaff(\""+data[i].commId+"\",\""+data[i].userName+"\",\""+data[i].diaryId+"\");'>回复</a>";
					if(accountId==data[i].accountId){
					main+="&nbsp;&nbsp;&nbsp;<a href=\"javascript:void(0);\" onclick='delcomments(\""+data[i].commId+"\",\""+data[i].diaryId+"\",\""+accountId+"\");'>删除</a>";
					}
			...
			for(var i=0;i<data.length;i++){
				for(var j=0;j<data.length;j++){
					...
					if(data[i].commId==data[j].commPid){
						levelcomm="<div id=\""+data[j].commId+"\" style=\"margin-left:6%;line-height:25px;\"><div style=\"width:100%;\"><div style=\"float:left;color:#428BCA;\">"+
						"<a href=javascript:showPersonal('"+data[j].accountId+"')>"+
						...
						if(accountId==data[j].accountId){
						levelcomm+="&nbsp;&nbsp;&nbsp;<a href=\"javascript:void(0);\" onclick='delcomments(\""+data[j].commId+"\",\""+data[j].diaryId+"\",\""+accountId+"\");'>删除</a>";
						}
...
function init(){
	var url=contextPath + "/diary/act/DiaryAct/selectDiaryAct.act";
...
$(function(){
	 ...
	 $(window).scroll(function (){
	        ...
	        if (scrollTop + windowHeight == scrollHeight) {
	        	...
	        	if(i==0){
	        		...
	        		url=contextPath+"/diary/act/DiaryAct/titletimeQueryDiaryAct.act";
	...
	onSelect: function(date){
		var titletime= date.format("yyyy-MM-dd");
		$('#presenttime').html(titletime);
		$('#time').show();
		i=1;
		var url=contextPath+"/diary/act/DiaryAct/titletimeQueryDiaryAct.act";
		page=1;
		parm={page:page,rows:rows,sort:sort,order:order,titletime:titletime};
		$('#content').html("");
		searchalldiary(url,parm);
