...
var currTime = '00:00:00';
	...
	function searchCalendar(){
		...
		var requestUrl = contextPath + "/calendar/act/CalendarAct/searchCycle.act"+par;
		doSearch(requestUrl);
	}
	function doSearch(requestUrl){
		...
		$('#myTable').datagrid({
			url:requestUrl,
			columns:[[
						{field:'START_DATE',title:'开始时间',width:'12%',align:'center',sortable:true,},
						{field:'END_DATE',title:'结束时间',width:'12%',align:'center',},
						{field:'CONTENT',title:'事务内容',width:'30%',align:'center'},
						{field:'CAL_TYPE',title:'事务类型',width:'12%',align:'center',
						...
						{field:'CAL_AFF_TYPE',title:'提醒类型',align:'center',width:'12%',
							formatter:function(value,rowData,rowIndex){
								if(rowData.REMIND_TYPE == '2'){
									return "按日提醒";
								}else if(rowData.REMIND_TYPE == '3'){
									return "按周提醒";
								}else if(rowData.REMIND_TYPE == '4'){
									return "按月提醒";
								}else if(rowData.REMIND_TYPE == '5'){
									return "按年提醒";
								}
							}
						},
						{field:'REMIND_TIME',title:'提醒时间',align:'center',width:'12%',
							formatter:function(value,rowData,rowIndex){
								if(rowData.REMIND_TYPE == '2'){
									return rowData.REMIND_TIME;
								}else if(rowData.REMIND_TYPE == '3'){
									return rowData.REMIND_DATE;
								}else if(rowData.REMIND_TYPE == '4'){
									return rowData.REMIND_DATE;
								}else if(rowData.REMIND_TYPE == '5'){
									var timeArr =  rowData.REMIND_DATE.split('-');
									return timeArr[0]+"月"+timeArr[1]+"日";
								}
							}
						},
		    ...
		    onLoadSuccess:function(data){  
	            if(data.total == 0){
	   	  			$('#myTable').datagrid('appendRow',{START_DATE:'<div style="text-align:center;color:red">没有相关记录！</div>' }).datagrid('mergeCells', { index: 0, field: 'START_DATE', colspan: 7 });
	 ...
	 "<table class=\"table table-striped\" width=\"100%\" align=\"center\">"+
       ...
       "</td>"+
	     ...
	     "<tr id=\"remindTimeTr\" style=\"display:none;\">"+
			...
			"<td nowrap align=\"left\" style=\"line-height:35px;\" ><div class=\"col-xs-12 form-group\"  >"+
             ...
             "<span id='years' style='display:none;'>"+
            	...
            	"</select>"+ 
             "</span>"+
         	"<input id=\"remindTime\" name=\"remindTime\" value='"+currTime+"' type=\"text\" onfocus=\"setRemindTime(this);\" style=\"width:25%;float:left;\" class=\"form-control BigInput easyui-validatebox\" validType=\"time[]\"   data-placement=\"right\" data-content=\"\" data-toggle=\"popover\" data-html=\"true\" data-original-title tile >"+
           	"&nbsp;为空为当前时间"+
			"</div></td>"+
	    "</tr>"+
		...
		"<td nowrap align=\"left\"><div class=\"col-xs-8 form-group\"  >"+
			...
			"<textarea  name=\"userName\" id=\"userName\" style=\"float:left;\" onClick=\"selectUser(['accountId','userName']);\" class=\"form-control easyui-validatebox BigTextarea SmallStatic\"  rows=\"2\" cols=\"35\" readonly=\"readonly\"></textarea>"+
