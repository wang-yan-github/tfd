...
function addresapply(){
	...
	var url=contextPath+"/officesupplies/act/OffresapplyAct/agentapplyAct.act";
	$.ajax({
		url:url,
		type:"POST",
		data:para,
		...
		success:function(data){
			if(data!=0){
				if(updateres($("resId").val(),$("#applyNum").val())){
...
function userNameval(){
	$('#resapplyform').bootstrapValidator('revalidateField', 'userName');
}
function blurjudge(){
