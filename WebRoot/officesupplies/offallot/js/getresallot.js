	...
	function getpitchresId(){
		...
		$.ajax({
			...
			success:function(data){
		   		...
		   		beforedispatchStaff=data.dispatchStaff;
		   		approveStaff=data.approveStaff;
		   		$("#beforelibraryId").val(data.libraryId);
		   		$("#beforeclassifyId").append("<option value=\""+data.classifyId+"\" selected='selected'> "+data.classifyName+"</option>");
		   		$("#resId").append("<option value=\""+data.resId+"\" selected='selected'> "+data.resName+"/库存"+data.beforeStock+"</option>");
		}
		});
	}
function getlibrary(){
		...
		$.ajax({
			...
			success:function(data){
				if(data!=0){
					...
					var url=contextPath+"/officesupplies/offallot/allotroght.jsp";
					window.location=url;
				}
		}
		});
	...
	$.ajax({
		...
		success:function(data){
			if(data!=0){
				...
				parent.document.getElementById("left").src= "/tfd/officesupplies/offallot/allotleft.jsp";
				var url=contextPath+"/officesupplies/offallot/allotright.jsp";
				window.location=url;
			}
	}
	});
